...
var autoScanGetTokens = function(template) {
    return new Promise(function(resolve, reject) {
       TemplateVar.set(template, 'autoScanStatus', TAPi18n.__('wallet.tokens.autoScan.status.downloadingList'));

        var tokenListURL = 'https://raw.githubusercontent.com/MyEtherWallet/ethereum-lists/649a45d58f2763f1b7a1e6039370cfb4d3d3c63a/tokens/tokens-eth.json';
        ...
        HTTP.get(tokenListURL, function(error, result) {
            ...
            TemplateVar.set(template, 'autoScanStatus',
            ...
            Tracker.flush();
            Meteor.defer(function(){ // defer to wait for autoScanStatus to update in UI first
                    ...
                    _.each(accounts, function(account) {
                        ...
                        try {
                            var promise = web3.eth.call({
                                ...
                                var statusString = TAPi18n.__('wallet.tokens.autoScan.status.checkingBalances', {
                                    number: numberOfBalancesToCheck - balancesChecked
                                });

                                if (tokensToAdd.length > 0) {
                                    statusString += ' (';
                                    statusString += TAPi18n.__('wallet.tokens.autoScan.status.found', {
